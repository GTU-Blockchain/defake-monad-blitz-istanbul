    ...
    <main className="w-full max-w-2xl mx-auto">
      ...
      <div className="text-center mb-10">
        ...
        <p className="text-muted-foreground text-sm max-w-md mx-auto leading-relaxed">
          Create and participate in secure, front-running resistant votes powered by commit-reveal on Monad.
        </p>
      </div>

      {/* Stats bar */}
      <div className="grid grid-cols-3 gap-3 mb-8">
        <div className="bg-background/60 backdrop-blur-md border border-border rounded-sm p-4 text-center">
          <svg viewBox="0 0 24 24" fill="none" className="w-5 h-5 text-accent mx-auto mb-2">
            <rect x="3" y="3" width="18" height="18" rx="2" stroke="currentColor" strokeWidth="2"/>
            <path d="M9 12l2 2 4-4" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
          <p className="text-xs text-muted-foreground font-mono uppercase tracking-wider">Protocol</p>
          <p className="text-sm font-mono text-foreground/90 mt-1">Commit-Reveal</p>
        </div>
        <div className="bg-background/60 backdrop-blur-md border border-border rounded-sm p-4 text-center">
          <svg viewBox="0 0 24 24" fill="none" className="w-5 h-5 text-accent mx-auto mb-2">
            <path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
          <p className="text-xs text-muted-foreground font-mono uppercase tracking-wider">Security</p>
          <p className="text-sm font-mono text-foreground/90 mt-1">Anti Front-Run</p>
        </div>
        <div className="bg-background/60 backdrop-blur-md border border-border rounded-sm p-4 text-center">
          <svg viewBox="0 0 24 24" fill="none" className="w-5 h-5 text-accent mx-auto mb-2">
          ...
          <p className="text-xs text-muted-foreground font-mono uppercase tracking-wider">Network</p>
          <p className="text-sm font-mono text-foreground/90 mt-1">Monad Testnet</p>
        </div>
      </div>
      ...
      {isConnected ? (
        showCreate ? (
          ...
          >
            <svg viewBox="0 0 24 24" fill="none" className="w-5 h-5 transition-transform group-hover:rotate-90 duration-300">
              <path d="M12 5v14M5 12h14" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/>
            </svg>
            <span className="font-mono text-sm tracking-widest uppercase">Create New Vote</span>
          </button>
        )
      ) : (
        <div className="w-full mb-8 py-6 px-6 bg-background/60 backdrop-blur-md border border-border rounded-sm text-center">
          ...
          <p className="text-foreground/80 font-mono text-sm mb-1">Wallet Not Connected</p>
          <p className="text-muted-foreground text-xs">Connect your wallet to create votes and participate.</p>
        </div>
      )}
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <span className="text-xs font-mono text-muted-foreground uppercase tracking-widest">Proposals</span>
        <div className="flex-1 h-px bg-border"></div>
      </div>
...
function ContractInfo({ contractAddress }: { contractAddress: `0x${string}` }) {
  ...
  return (
    <div className="flex items-center gap-3 mb-6 p-4 bg-muted/20 border border-border rounded-sm">
      ...
      <div className="min-w-0">
        <p className="text-xs text-muted-foreground font-mono uppercase tracking-wider">Contract</p>
      ...
      {!isConnected ? (
        <div className="bg-background/60 backdrop-blur-md border border-border p-10 rounded-sm shadow-lg text-center relative overflow-hidden">
          ...
          <div className="w-14 h-14 rounded-full bg-muted/50 border border-border flex items-center justify-center mx-auto mb-4">
            <svg viewBox="0 0 24 24" fill="none" className="w-7 h-7 text-muted-foreground">
              <rect x="3" y="11" width="18" height="11" rx="2" stroke="currentColor" strokeWidth="2"/>
              <path d="M7 11V7a5 5 0 0110 0v4" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/>
            </svg>
          </div>
          <p className="text-foreground/80 font-mono text-sm mb-1">Wallet Not Connected</p>
          <p className="text-muted-foreground text-xs">Connect your wallet to participate in this vote.</p>
        </div>
      ) : (
